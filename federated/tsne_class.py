...
import json
from utils.util import agg_feat, prepare_data, agg_rep
...
if __name__ == '__main__':
    ...
    for client_idx in range(client_num):
        ...
        feat, label = agg_feat(args, server_model, test_loaders[client_idx], device)
    ...
    feat_tsne = manifold.TSNE(n_components=2, init='random', random_state=5, verbose=1).fit_transform(all_feat)
    ...
    # plt.xticks([])
    # plt.yticks([])
    sns_plot.get_figure().savefig(f"./img/tsne_feat_{exp_folder}_{args.mode}.png")
